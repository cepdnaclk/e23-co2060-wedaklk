...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = useMemo(() => {
    ...
    if (isAuthenticated) {
      items.push({ href: '/dashboard/jobs/create', label: 'Post a Job' });
    }
    return items;
  }, [isAuthenticated]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* Authenticated: Show profile avatar + dropdown */}
              {isAuthenticated ? (
                <div className="relative">
                  ...
                  {profileOpen && (
                    <div className="absolute right-0 mt-3 w-64 rounded-2xl border border-gray-100 bg-white shadow-xl p-4 space-y-4">
                      ...
                      <button
                        onClick={() => signOut({ callbackUrl: '/dashboard/jobs' })}
                ...
                <div className="flex items-center gap-2">
                  <Link
                    href="/auth"
                    className="hidden sm:inline-flex items-center gap-2 rounded-full border border-emerald-200 px-4 py-2 text-sm font-medium text-emerald-700 hover:bg-emerald-50 transition-colors"
                  >
                    <LogIn size={16} />
                    Login
                  </Link>
                  <Link
                    ...
                    className="hidden sm:inline-flex items-center gap-2 rounded-full bg-emerald-500 text-white px-4 py-2 text-sm font-medium shadow-lg hover:bg-emerald-600 transition-colors"
                  >
                    <UserPlus size={16} />
                    Register
                  </Link>
                  ...
                  <Link
                    href="/auth"
                    className="sm:hidden inline-flex items-center justify-center p-2 rounded-full border border-emerald-200 text-emerald-700 hover:bg-emerald-50"
                    aria-label="Login"
                  >
                    <LogIn size={20} />
                  </Link>
                  <Link
                    href="/auth?mode=register"
                    className="sm:hidden inline-flex items-center justify-center p-2 rounded-full bg-emerald-500 text-white hover:bg-emerald-600"
                    aria-label="Register"
                  >
                    <UserPlus size={20} />
                  </Link>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-gray-100 bg-white">
            <div className="px-4 py-3 space-y-2">
              ...
              {/* Mobile menu: show Login/Register links for unauthenticated users */}
              {!isAuthenticated && (
                <>
                  <Link
                    href="/auth"
